	...
	for( int j=0 ; j<height ; j++ )
	{
		for( int i=0 ; i<width ; i++ )
		{
			try
			{
				...
				std::vector<Ray3D> rayVector = std::vector<Ray3D>();
				_globalData.camera.getRay_AA(i, height - j - 1, width, height, rayVector);
				Point3D c1 = getColor(rayVector[0], rLimit, Point3D(cLimit, cLimit, cLimit));
				Point3D c2 = getColor(rayVector[1], rLimit, Point3D(cLimit, cLimit, cLimit));
				Point3D c3 = getColor(rayVector[2], rLimit, Point3D(cLimit, cLimit, cLimit));
				Point3D c4 = getColor(rayVector[3], rLimit, Point3D(cLimit, cLimit, cLimit));
				Point3D c = (c1 + c2 + c3 + c4) / 4;
