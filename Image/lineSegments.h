...
namespace Image
{
	...
	// Helper functions
	private:
		/** This method rotates the given vector by a given angle in degrees. */
		static Util::Point2D rotateVector(Util::Point2D v, double angle);

		/** This method normalizes a vector. */
		static Util::Point2D normalizeVector(Util::Point2D v);

		/** This method computes the length of a vector. */
		static double getLength(Util::Point2D v);

		/** This method returns the vector pointing from the start to the end of the linesegment.*/
		Util::Point2D vector(void) const;

		/** This method returns the vector pointing from one point to the second point. */
		static Util::Point2D vector(Util::Point2D p1, Util::Point2D p2);
	};
